...
def rand_point(ranges, ID=0, gases=[de.par.index['AR']], first_step=0.1):
    print('Warning! Rand_point is not updated! It should not be used!')
    point = dict(ID=ID)
    for key in ranges:
        if len(ranges[key]) < 2:
...
def fix_starter_point(ranges, ID, cpar, gases=[de.par.index['AR']], first_step=0.1):
    point = dict(ID=ID)
    for key in cpar:
        point[key] = cpar[key]
    point['gases'] = gases
...
def in_range(point, ranges, start_point):
    for key in ranges:
        if key in ['ID', 'gases']:
            continue
        if len(ranges[key]) < 2:
            if point[key] != ranges[key][0] and len([start_point[key]]) != 1:
                return False
        elif key == 'fractions': 
...
def gradient_method(ranges, to_optimize, start_point, max_steps=100, first_step=0.1, min_step=0.02, decay=0.6, gamma=1.0, t_int=np.array([0.0, 1.0]), LSODA_timeout=30, Radau_timeout=300):
    ...
    keys = ['R_E', 'ratio', 'P_amb', 'alfa_M', 'T_inf', 'surfactant', 'fractions', 'freq', 'p_A', 'freq1', 'freq2', 'pA1', 'pA2', 'theta_phase', 'mu_L', 'c_L']
    ...
    while step_num < max_steps and current_step > min_step:
        ...
        for i in range(len(keys)):   # make two steps in all directions 
            ...
            if key != 'fractions':
                ...
                if in_range(new_point, ranges, start_point):
                    ...
                    print('ID of start_point: '+str(start_point['ID'])+', key:'+str(key));print('ranges:');print(ranges);print('new_point:');print(new_point)
                    notinrange = 1
