...
def _pressure(t, R, R_dot, mu_L, n_liquid, surfactant, rho_L, p, p_dot, P_amb, args):
    (p_Inf, p_Inf_dot) = Excitation(t, P_amb, args)
    if(R_dot != 0.0):
        p_L = p - (2.0 * surfactant * par.sigma / R + 4.0 * mu_L * (2.0 * 3.0**0.5 * abs(R_dot))**(n_liquid-1.0) * R_dot /(n_liquid* R**n_liquid))  #p - (2.0 * surfactant * par.sigma + 4.0 * mu_L * R_dot) / R
    else:
        p_L = p - (2.0 * surfactant * par.sigma / R)  #p - (2.0 * surfactant * par.sigma + 4.0 * mu_L * R_dot) / R    
    p_L_dot = p_dot + 2.0 * surfactant * par.sigma * R_dot / R**2.0 + 4.0 * mu_L * (2.0 * 3.0**0.5)**(n_liquid-1.0) * abs(R_dot)**(n_liquid+1.0) / (R**(n_liquid+1.0)) #p_dot + (+2.0 * surfactant * par.sigma * R_dot + 4.0 * mu_L * R_dot ** 2) / (R ** 2)
...
def _f(t, x, R_E, P_amb, alfa_M, Gamma, sigma_evap, T_inf, surfactant, P_v, C_4_starred, mu_L, n_liquid, rho_L, c_L, thermodynamicalcase, ex_args, extra_dims=0): 
...
# d/dt R_dot
    (delta, delta_dot) = _pressure(t=t,
        R=R, R_dot=R_dot, mu_L=mu_L, n_liquid=n_liquid, surfactant=surfactant, rho_L=rho_L,
        p=p, p_dot=p_dot, P_amb=P_amb, args=ex_args
    )   # delta = (p_L-P_amb) / rho_L
    
    abs_R_dot_eff = max(abs(R_dot),1.0e-300)
    Nom = (1.0 + R_dot / c_L) * delta + R / c_L * delta_dot - (1.5 - 0.5 * R_dot / c_L) * R_dot ** 2
    if (R_dot != 0.0):
        Den = (1.0 - R_dot / c_L) * R + 4.0 * mu_L * (2.0 * 3.0**0.5 * abs(R_dot))**(n_liquid-1.0)/ (c_L * rho_L * R**(n_liquid-1.0)) #Den = (1.0 - R_dot / c_L) * R + 4.0 * mu_L / (c_L * rho_L)
    else:
        Den = (1.0 - R_dot / c_L) * R
...
@njit(float64(float64, float64[:], float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64, float64[:], int64))
def stop_event(t, x, R_E, P_amb, alfa_M, Gamma, sigma_evap, T_inf, surfactant, P_v, C_4_starred, mu_L, n_liquid, rho_L, c_L, thermodynamicalcase, ex_args, extra_dims=0):
    # Ha minden derivált abszolút értéke kisebb, mint 10e-10, az esemény bekövetkezik
    if(t>1.0e-3):
        dxdt = _f(t, x, R_E, P_amb, alfa_M, Gamma, sigma_evap, T_inf, surfactant, P_v, C_4_starred, mu_L, n_liquid, rho_L, c_L, thermodynamicalcase, ex_args, extra_dims=0)
        ...
        if(enable_time_evaluation_limit):
            num_sol = func_timeout( # timeout block
                ...
                kwargs=dict(fun=_f, t_span=t_int, y0=IC, t_eval=t_eval, method='LSODA', atol = 1e-10, rtol=1e-10, first_step=first_step, events=stop_event,# solve_ivp()'s arguments
                        args=(cpar.R_E, cpar.P_amb, cpar.alfa_M, cpar.Gamma, cpar.sigma_evap, cpar.T_inf, cpar.surfactant, cpar.P_v, cpar.C_4_starred, cpar.mu_L, cpar.n_liquid, cpar.rho_L, cpar.c_L, cpar.thermodynamicalcase, ex_args, extra_dims) # _f()'s arguments
            ))
        else:
            num_sol = func_timeout( # timeout block
                LSODA_timeout, solve_ivp,
